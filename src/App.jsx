...
function App() {
  ...
  return (
    <div className='directory bg-slate-100 px-0'>
      <h1 className="text-center text-2xl md:text-4xl font-semibold text-white p-9">User Directory</h1>
      ...
      <div className="min-w-full min-h-screen p-5">
        ...
        <div className='w-full flex justify-center mt-5 text-sm'>
          <select onChange={changeCountry} className='md:w-[10%] mx-auto h-9 md:h-11 text-sm border-[1px] px-2 border-stone-500 rounded-md outline-none'>
            <option value="">Country</option>
            {countries.map((nat, index) => (
              <option key={index} value={nat} className='text-sm md:text-lg'>{nat}</option>
            ))}
          </select>
        </div>

        <div className="w-full text-xl flex flex-col flex-wrap pt-9 items-center gap-3">
          <h3 className='text-stone-800 text-base md:text-lg md:text-white backdrop-blur-sm'>Total results found: {users.length}</h3>
          {currentUsers.map((user) => (
            <div key={user.email} className="w-[98%] md:w-[50%] mx-auto py-1 md:py-4 px-1 md:px-6 border-2 bg-white border-stone-300 hover:border-[#bbbbbb] rounded-xl flex shadow-sm hover:shadow-sm hover:scale-[1.005] md:hover:shadow-lg transition-all overflow-x-hidden gap-x-4 items-center md:gap-x-9">
              ...
              <div className="flex flex-col text-sm md:text-lg">
                ...
                <span className="flex items-start gap-x-2"><GoLocation /> {user.location.city}, {user.location.state}, {user.location.country}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="w-full flex justify-center mt-10 md:mt-28">
          {Array.from({ length: totalPages }, (_, index) => (
            <button
              ...
              className={`px-3 md:px-4 py-1 md:py-2 mx-1 border rounded-full ${currentPage === index + 1 ? 'bg-stone-800 text-white' : 'bg-white border-stone-300'}`}
            >{index + 1}</button>
          ))}
        </div>
      </div>
